...
function get_resp_list_sql ($me_param) {				// responder list sql - 		
	...
	return "					
		...
		ORDER BY `mine` ASC, `resp_type` ASC, `handle` ASC
		";			
...
<head>
	<meta charset="utf-8" />
	<title>Tickets SP <?php echo get_text("Responders");?></title>
...
<?php	
			...
			echo "\n<br/><br/><center><h2>Selected " . get_text("Responder") . " (# {$the_nth} of {$the_count} )</h2>\n";		
			...
			echo "<table border=1 id = 'table' class='single table-striped' >\n";
					...
					if ( ( !empty($row[$i] ) ) &&  ( !is_null ( $row[$i] ) ) ) {			
						...
						switch ( strtolower(mysql_field_name($result, $i) ) ) {
							...
							case "unit_status":
								...
								echo "<tr {$the_click_str}><td>Dispatch/Status:</td><td>{$the_val}<img style = 'margin-left:20px;' src = './images/go-right.png'/</td></tr>";								
								break;
		...
		if ( intval ( $_SESSION['SP']['do_map'] ) == 1 ) {
			echo "<tr><td colspan=2 align = 'center'><br />";

			echo "<input type = 'button' value = 'to Map' onclick = 'call_map();' />";
		
//		if ( ! ( ( sp_is_guest() ) || ( sp_is_member() ) )  ) {						// disallow
//			echo "<input type = 'button' value = 'Update status' onclick = '' style = 'margin-left:60px;' />";
//			}
			echo "</td></tr>\n";
			}		// end do_map
...
//	====== ====== ====== ====== ====== ====== ====== ====    LIST    ====== ====== ====== ====== ====== ====== ====== ====== 			
		...
		function sp_show_resp_list($res, $hides, $proc, $top_row_txt = "" ) {					// returns table for display - 10/26/2013
			$ini_array = parse_ini_file("./incs/sp.ini");
			$types_lim = ( array_key_exists ( "unit_count", $ini_array ) ) ? intval ( $ini_array['unit_count'] ) : "15" ;
			...
			while ($in_row = stripslashes_deep(mysql_fetch_assoc($res))) {			// counts by type
				if ( ! ( array_key_exists($in_row["resp_type"], $counts ) ) )  { $counts[$in_row["resp_type"]] = 0 ;}
				$counts[$in_row["resp_type"]]++;
				}
			$i = 0;				// row index
		
			mysql_data_seek($res, 0) ;										// pick up 1st row
			$in_row = stripslashes_deep(mysql_fetch_assoc($res));
			$this_type = $in_row['resp_type'];
		
			mysql_data_seek($res, 0) ;										//start at 1st row
		 	while ($in_row = stripslashes_deep(mysql_fetch_array($res))) {					// 
				$this_class = (  intval ( $in_row['mine'] ) == 1 ) ? "me" 					: $in_row['resp_type'];
				if ( !$do_types ) { 
					$this_style =  "display: '';"; 
					}
				else {
					$this_style = (  intval ( $in_row['mine'] ) == 1 ) ? "display: '';" : "display: none;";
					}
		
			 	if ( ( ($do_types) && $in_row['resp_type'] != $this_type ) ) {				// we have a type change - do button		
			 		$out_str .=  "\n<tr class = 'perm' onclick = \"changeRows('table', '{$this_class}');\">
				 			<td colspan=9 align = 'center'><button type='button'><b>{$in_row['resp_type']}</b></button> ({$counts[$in_row['resp_type']]})</td></tr>\n\n";
				 	$this_type = $in_row['resp_type'];
			 		}
			...
			return $out_str;
			}		// end end end end end end end end end end function sp_show_resp_list()
...
?>
	<script>
		DomReady.ready(function() {						// <?php echo __LINE__ . "\n\n";?>
<?php
	@session_start();
...
<?php
			...
			if ( mysql_num_rows($result) == 0 ) {
				...
				$hides = array("mine", "handle", "the_group", "as_of", "icon", "assign_id", "id", "lat", "lng", "APRS", "instamapper", "open GTS", "locateA", "g tracker", "t tracker", "g Latitude");		// hide these columns

				$top_row = get_text("Responders") . " (" . mysql_num_rows($result) . ")  - <i>click/tap for details</i>";
				
				echo sp_show_resp_list($result, $hides, basename(__FILE__), $top_row ) . "\n" ;	// do the list

				echo "</div><br /><br /><center>";			// show bottom rows
				}
			} 		// end else {} ====== ====== ====== ====== ====== ====== ====== ====== ==
