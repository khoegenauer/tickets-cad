...
1/3/2014 - revised function log_remotes_err(), applied to several handlers
...
function log_remotes_err($message) {					// error logger 		// 1/3/2014
	@session_start();
	snap (__LINE__, $message);
	if ( ! (array_key_exists ( $message, $_SESSION ) ) ) {		// limit to once per session per message	
...
//				print "-error " . __LINE__;		// @fopen fails
				$err_msg = basename(__FILE__) . "/" .  __FUNCTION__ . "/" . __LINE__;		// 1/3/2014
				log_remotes_err($err_msg);
				}
			}
...
//	                   000000000000000000000                    11111111   22222222
	...
	if (count($ogts_info) != 3) {
		log_remotes_err("OpenGTS setting 'ogts_info' format error");
		return FALSE;
		}
...
//			log_remotes_err("OpenGTS connection attempt failed");
			$err_msg = basename(__FILE__) . "/" .  __FUNCTION__ . "/" . __LINE__;		// 1/3/2014
			log_remotes_err($err_msg);
			return FALSE;
			}
...
//		log_remotes_err("OpenGTS JSON data format error");
		$err_msg = basename(__FILE__) . "/" .  __FUNCTION__ . "/" . __LINE__;		// 1/3/2014
		log_remotes_err($err_msg);
		return FALSE;
		}
